...
import { getPropValue, runCheck, asyncCall } from "./evaluate.js"
...
export default async (options) => {
    ...
    for (const [load, displayFile] of files) {
        ...
        // collection.executionTime = mapsum(collection.sections, s => s.executionTime)
        // collection.checkTime = mapsum(collection.sections, s => s.checkTime)
        hooks.collectionEnd(collection)

        if (failAction === "afterCollection") {
            await hooks.teardown(runScope)
            return {
    ...
    const suiteResults = {
        ...
        // checks: results.map(file => file.checks).flat(),
